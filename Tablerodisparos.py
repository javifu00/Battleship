...
def main():
    ...
    while sis:
        ubicar_naves()
        print("Las naves estan en: {}".format(lista_ubicacion_barco))
        jugar = int(input("ingrese: 1) Jugar 2) Salir : "))
        if jugar == 1:
            print("El juego ha empezado, suerte")
            mostrar_tablero(tablero)
            juego()
            print("Has destruido todos los barcos, felicidades venciste al programa hecho por un chamo :)")
            nueva_partida = int(input("Desea jugar otra vez: 1) Si 2) No: "))
            if nueva_partida == 1:
                sis = True
            else: sis = False
        else: sis = False
